...
// Cartes de départ offertes à tout nouveau joueur (ou joueur sans collection)
const STARTER_CARDS = [
...
const SHOP_ITEMS = {
  ...
  "pack_senegal": { cost: 200, lives: 0, duration: null, hints: 0, pack: "senegal" },
};
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET" && action === "stats") {
    ...
    // Injecter les cartes de départ si la collection est vide
    // S'applique aux nouveaux joueurs ET aux inscrits existants sans cartes
    if (!stats.cards || stats.cards.length === 0) {
      stats = await GameStats.findOneAndUpdate(
        { userId: decoded.id },
        { $set: { cards: STARTER_CARDS } },
        { new: true }
      );
    }
